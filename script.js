...
function playAnim() {
    if (track > 29) {   // 29 is last coz we count from 0, and also track = 30 coz we increment after the anim
        alert("Finished!")
        return
    }
...
function setNewTrack() {
    if (track < 30) {
        ...
        // replace coz you can't have ? in filename? didn't knwo that ngl
        document.querySelector("img").src = `./track_images/${newTrack.replace(/\?/g, '_')}.png`
        track++
    }
}
